...
def get_raw_aapt(apk_path: str) -> str:
    try:
        return subprocess.run(
            [get_aapt_path(), 'd', 'badging', apk_path],
...
class ApkInfo:
    __slots__ = ['package_name', 'version_code', 'version_name', 'min_sdk_version', 'target_sdk_version',
                 ...
                 'supports_screens', 'supports_any_density', 'langs', 'densities', 'abis']
    package_name: str
    version_code: int
...
def get_apk_info(apk_path: str, as_dict: bool = False) -> Union[ApkInfo, Dict]:
    data = {field.name: re.findall(getattr(Patterns, field.name),
                                   get_raw_aapt(apk_path)) for field in dataclasses.fields(ApkInfo)}
    info = ApkInfo(
        ...
        install_location=InstallLocation(data['install_location'][0]) if data.get('install_location') else None,
    ...
    return info if not as_dict else dataclasses.asdict(info)
